...
def main():
    PROP = ["p_3111_1_Glass-transition-temp",
            "p_3154_1_Half-time-of-crystallization",
            "p_3211_1_Specific-heat-capacity(Cp)",
            "p_4133_1_Electric-conductivity",
            "p_5111_1_Solubility-parameter",
            "p_5141_1_Gas-permeability-coeff(P)"]
...
def main2(prop):
    ...
    indir = "/mnt/c/Users/a015968/Oka/Oka_Subject/159_unit-converter/out" + "/" + prop
    outdir = indir
    ...
    # Input a unit list
    indir_list = indir
    unit_list = "UNITLIST_" + prop[9:] + ".dat"
    INPUT = src_1.input_data(indir_list + "/" + unit_list, tab=0, blanc_line=1)                                # import
    ...
    # Unit conversion
    infname = ["CA_" + prop[9:] + ".dat", "05_extract.dat"]
    INPUT = src_1.input_data(indir + "/" + infname[0], tab=1, blanc_line=1)                                    # import
    ...
    for i, x in enumerate(DATA):
        # if i > 1: break
        prop_specifier, value, related_tcd, tcaption, tfoot = x[3], x[4], x[8], x[9][10:], x[10][11:]
        ...
        for j, y in enumerate(TEXT):
            UNIT, COEFF, recog_unit = recognize_unit(y, UNIT_GENE, COEFF_GENE, unit_list)                      # def
            if recog_unit == 1:
                place = PLACE[j]
                if j == 4: break
                ...
                """
                    ...
                    for k, z in enumerate(TEXT):
                        if k in [j]: continue
                        power_of_ten_2, power_recognized_2 = src_2.recognize_power_of_ten(z, prop=1)               # import
                        if power_of_ten_2 != "_":
                            if z == 0:    # prop-specifier
    ...
    HEADER[2].extend(["power-of-ten-in-value/others", "unit", "coeff", "position-describing-unit", "value-clean", "value-convert"])
    if eval == 1:
        HEADER[1].extend(["EVALUATION", "", "", ""])
        HEADER[2].extend(["TF-power-of-ten", "TF-unit", "TF-value-clean", "TF-value-convert"])
    OUTPUT = src_1.edit_for_output(HEADER, OUTPUT)                                                             # import
    outfname = "OUT_" + prop[9:] + ".dat"
    src_1.output(outdir + "/" + outfname, OUTPUT)                                                              # import

    HEADER = [["unit", "coeff"]]
    OUTPUT = edit_for_output_2(HEADER, UNIT_GENE, COEFF_GENE)                                                  # def
    outfname = "OUT_generated-unit_" + prop[9:] + ".dat"
    src_1.output(outdir + "/" + outfname, OUTPUT)                                                              # import
...
def recognize_unit(text, UNIT_GENE, COEFF_GENE, unit_list):
    import re

    UNIT = ["_" for a in UNIT_GENE]
    if unit_list == "temperature": COEFF = [0 for a in UNIT_GENE]
